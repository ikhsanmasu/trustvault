    ...
    <div className="flex-1 flex items-center justify-center p-8">
      <div className="text-center max-w-md">
        ...
        <p className="text-sm text-muted-foreground mb-6">
          The assistant searches your vault and answers with citations to the
          exact passage — try one of these to start:
        </p>
        ...
        <div className="space-y-2 text-left">
          {[
            ...
            <button
              ...
              className={cn(
                "w-full rounded-xl border border-border bg-card px-3.5 py-2.5 text-left",
                "text-[13px] leading-relaxed text-muted-foreground",
                "transition-all duration-200 hover:border-secondary/40 hover:text-foreground hover:shadow-elevation-1",
              )}
            >
              &ldquo;{example}&rdquo;
            </button>
...
function KnowledgeBasePanel({
  ...
  return (
    <div className="hidden lg:flex w-[280px] shrink-0 border-l border-border/60 bg-muted/30 flex-col min-h-0">
      {/* ---- Header ---- */}
      ...
      <div className="flex-1 flex flex-col min-h-0">
        ...
        {/* ---- System Prompt editor (advanced, collapsed by default) ---- */}
        <details className="shrink-0 border-t border-border/40 px-3 py-2.5 group">
          <summary className="flex cursor-pointer list-none items-center justify-between text-xs font-medium text-muted-foreground hover:text-foreground transition-colors [&::-webkit-details-marker]:hidden">
          ...
          <Textarea
            ...
            rows={5}
            className="mt-2 resize-none text-xs leading-relaxed min-h-[100px]"
...
function AssistantContent() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-10rem)] lg:h-[calc(100vh-8rem)]">
      {/* ---- Header ---- */}
      <div className="shrink-0 flex items-end justify-between gap-3 pb-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-foreground">
            Assistant
          </h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Grounded answers from your vault — every claim cited.
          </p>
        </div>

        {/* Toggle knowledge base panel (panel is lg+ only) */}
        <button
          type="button"
          onClick={handleToggleKnowledgePanel}
          className={cn(
            "hidden lg:inline-flex h-9 items-center gap-1.5 rounded-xl border px-3 text-sm font-medium transition-colors whitespace-nowrap shadow-elevation-1",
            showKnowledgePanel
      ...
      {/* ---- 3-panel layout ---- */}
      <div className="flex flex-1 min-h-0 rounded-2xl border border-border/60 bg-card overflow-hidden shadow-elevation-1">
        {/* ---- Left: Session sidebar (md+) ---- */}
        <div className="hidden md:flex w-[220px] shrink-0 border-r border-border/60 bg-muted/30 flex-col">
          <ChatSessionList
            sessions={sessions}
        ...
        <div className="flex-1 flex flex-col min-w-0">
          {!currentSessionId && messages.length === 0 && !isStreaming ? (
            <EmptyState onAsk={handleSendMessage} />
          ) : (
            <>
              {/* Error banner */}
              {chatError && (
          ...
          {/* ---- Answer scope indicator ---- */}
          <div className="shrink-0 flex items-center gap-1.5 border-t border-border/40 bg-muted/20 px-4 py-1.5 text-[11px] text-muted-foreground">
            <span className={cn("h-1.5 w-1.5 rounded-full shrink-0", selectedDocIds.size > 0 ? "bg-secondary" : "bg-success")} />
            {selectedDocIds.size > 0
              ? `Answering from ${selectedDocIds.size} selected document${selectedDocIds.size !== 1 ? "s" : ""}`
              : "Searching across all your documents"}
            <button
              type="button"
              onClick={() => setShowKnowledgePanel(true)}
              className="ml-1 hidden lg:inline font-medium underline underline-offset-2 hover:text-foreground transition-colors"
            >
              Change
            </button>
          </div>
